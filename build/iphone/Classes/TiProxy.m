...
//Rare exceptions to indicate a bug in the _businesscard code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_businesscard = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_businesscard]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.businesscard.module"
